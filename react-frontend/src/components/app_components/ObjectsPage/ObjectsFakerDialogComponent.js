...
import objectsFakerFactory from "./objectsFakerFactory";
...
const ObjectsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = objectsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("objects").create(elem));
...
export default connect(mapState, mapDispatch)(ObjectsFakerDialogComponent);
